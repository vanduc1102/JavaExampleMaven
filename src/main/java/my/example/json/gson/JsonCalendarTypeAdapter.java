...
public class JsonCalendarTypeAdapter
    implements JsonDeserializer<Calendar>, JsonSerializer<Calendar> {
  ...
  /**
   * Register an adapter to manage the calendar types that are received as long values on json
   * string. First the method try to deserialize the json value with ISO8601 long. If the json value
   * is unparsable, the method again parse json value with ISO8601 timezone. it makes the method
  ...
  public Calendar deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    throw new JsonParseException(
        "Unparseable date: \""
            + json.getAsString()
            + "\". Supported formats: "
            + Arrays.toString(DESERIALIZE_DATE_FORMAT));
  }

  /**
   * Register an adapter to manage the calendar types and serialize calendar as ISO 8601 when
   * creating a json string. Always serialize the date with ISO long pattern.
   *
   * @param calendar
   * @param typeOfT
   * @param context
